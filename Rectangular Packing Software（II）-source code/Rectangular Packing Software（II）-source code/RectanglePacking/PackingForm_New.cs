...
namespace RectanglePacking
{
    public partial class PackingForm_New : Form
    {
        ...
        public int RandomCount;  //设定追溯循环次数
            ...
            foreach (int index in UnPack_Index)
            {
                ...
                else if (RectangList[index][0] == W || RectangList[index][1] == W)
                {
                    ...
                    if (RectangList[index][0] == W & b_line.Start_Point.Y + RectangList[index][1] <= H_Max)
                    {
                        ...
                        else if (RectangList[TempList_4[0].Index][2] < RectangList[index][2]) //找面积最大
                        {
                            TempList_4[0].Index = index;
                            TempList_4[0].Rotate = 0;
                        }
                ...
                else if (RectangList[index][0] < W || RectangList[index][1] < W)
                {
                    ...
                    if (RectangList[index][0] < W & b_line.Start_Point.Y + RectangList[index][1] <= H_Max)
                    {
                        ...
                        else if (RectangList[TempList_7[0].Index][2] < RectangList[index][2]) //找面积最大
                        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            XmlNode node2 = myxml.DocumentElement.ChildNodes[1];
            RecoverCount = Convert.ToInt32(mynode1.InnerText);
            RandomCount = Convert.ToInt32(node2.InnerText);

            string filepath_1 = Application.StartupPath + "\\Result_input.txt";
            StreamReader sr_1 = new StreamReader(filepath_1);
            string str_new;
            while ((str_new = sr_1.ReadLine()) != null)
            {
                string[] Str_new = str_new.Split(' ');
                if (Str_new[1] == Str_new[2])  //目前已经是最优解
                {
                    //保存结果
                    string path_1 = Application.StartupPath + "\\Result.txt";
                    StreamWriter sw_1 = File.AppendText(path_1);
                    string rr_1 = Str_new[0] + " " + Str_new[1] + " " + Str_new[2];
                    sw_1.WriteLine(rr_1);
                    sw_1.Flush();
                    sw_1.Close();
                ...
                for (int index_00 = 1; index_00 <= RandomCount; index_00++)
                {
                    //数据初始化
                    ...
                    #region 读取数据文件txt
                    StreamReader sr = new StreamReader(Application.StartupPath + "\\data3\\" + Str_new[0]);
                    string str;
                    int index = -2;  //自动生成编号 
                    int area = 0;
                    ...
                    int index_2 = 0;
                    int random_count = 1;
                    while (random_count <= RectangList.Length)
                    {
                        index_1 = rd.Next(RectangList.Length);
                        index_2 = rd.Next(RectangList.Length);
                        //排除First矩形的索引位置
                        if (index_1.ToString() != Str_new[3] & index_2.ToString() != Str_new[3])
                        {
                    ...
                    #region 循环迭代
                    int first_index = Convert.ToInt32(Str_new[3]);
                    ...
                    {
                        if (Result_H < goodvalue)
                        {
                            goodvalue = Result_H;
                        }
                        if (Result_H > badvalue)
                        {
                            badvalue = Result_H;
                        }
                        sumvalue = sumvalue + Result_H;

                    }

                    //if (Result_H == LB)  //找到理论最优解
                    //{
                    //    result_0000 = Result_H;
                    //    break;
                    //}
                    //else if (Result_H < result_0000)
                    //{
                    //    result_0000 = Result_H;
                    //}
                ...
                //计算平均值
                averagevalue = Convert.ToSingle(sumvalue) / Convert.ToSingle(RandomCount);
                ...
                string rr = Str_new[0] + " " + LB.ToString() + " " + goodvalue.ToString() + " " + badvalue.ToString() + " " + averagevalue.ToString("0.00") + " " + ts3.TotalSeconds.ToString();
                sw.WriteLine(rr);
                sw.Flush();
                sw.Close();
